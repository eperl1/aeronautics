...
# < /dev/null prevents the server from instantly shutting down due to an empty terminal pipe
java -Xmx6G -Xms2G -XX:+UnlockExperimentalVMOptions -XX:+UseShenandoahGC -XX:ShenandoahUncommitDelay=1000 -XX:ShenandoahGuaranteedGCInterval=10000 @user_jvm_args.txt @libraries/net/neoforged/neoforge/21.1.230/unix_args.txt "$@" --nogui < /dev/null &
JAVA_PID=$!
